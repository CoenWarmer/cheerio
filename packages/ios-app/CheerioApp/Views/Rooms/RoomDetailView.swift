...
struct RoomDetailView: View {
    ...
    private func joinRoomAndSubscribe() async {
        guard !hasJoinedRoom else { return }
        hasJoinedRoom = true
        
        // Join the room
        do {
            let roomService = RoomService(supabase: appState.supabase)
            try await roomService.joinRoom(slug: room.slug)
        } catch {
            print("Failed to join room: \(error)")
        }
        
        // Subscribe to chat (use slug for API, but room.id for realtime channel)
        await chatService.subscribeToRoom(roomId: room.id, roomSlug: room.slug) { audioUrl in
            Task {
                await audioService.playAudio(from: audioUrl)
            }
        }
    ...
    private func updatePresence() async {
        do {
            let url = URL(string: "\(Config.apiBaseURL)/api/rooms/\(room.slug)/presence")!
    ...
    private func toggleTracking() {
        if locationService.isTracking {
            ...
            Task {
                await locationService.startTracking(roomSlug: room.slug)
            }
        }
    }
    ...
    private func sendMessage() {
        ...
        Task {
            do {
                ...
                try await chatService.sendMessage(roomSlug: room.slug, content: newMessage, location: location)
                newMessage = ""
            } catch {
    ...
    private func stopRecordingAndSend() {
        ...
        Task {
            do {
                let uploadResult = try await audioService.uploadAudio(fileURL: fileURL, roomSlug: room.slug)
                
                // Send message with audio attachment (matching web app format)
                let url = URL(string: "\(Config.apiBaseURL)/api/rooms/\(room.slug)/messages")!
                var request = URLRequest(url: url)
                request.httpMethod = "POST"
                request.setValue("application/json", forHTTPHeaderField: "Content-Type")
    ...
    private func handleEmojiTap(_ emoji: String) {
        Task {
            do {
                ...
                try await chatService.sendMessage(roomSlug: room.slug, content: emoji, location: location)
            } catch {
